...
solver::solver(CircuitGraph &graph) {
  ...
  for (int i = 0; i < noPO_lines_name_size; i++) {
    ...
    if ((temp->destination_gates.size() > 10) || (temp->source->get_type() == Gate::Type::Input))
...
int solver::watch_BCP(CircuitGraph &graph, int decision_line) 
{
  ...
  int dir_idx = 0;
  int indir_idx = 0;
  while (dir_idx < bcp_vec.size() || indir_idx < bcp_vec.size())  // || or &&
  {
    //直接蕴含的所有推理
...
int solver::conflict_backtrack(int decision_line, CircuitGraph &graph, std::vector<int> &decision_line_name, int confict_nums) {
  ...
  // std::cout << " ---------------- " << std::endl;
  // for(const auto& c : decision_line_name)
  //   std::cout << c << "-";
  // std::cout << std::endl;
  // for(const auto& c : conflict_line)
  //   std::cout << c << " ";
  // std::cout << std::endl;
  // std::cout << " ///////////////// " << std::endl;
  ...
  else  // learnt_gate.size()>1
  {
    ...
    for (int i = 0; i < learnt_gate.size(); ++i) {
      ...
      //std::cout << learnt_gate[i] << "-*-" << ls.at(learnt_gate[i]).level << std::endl;
    }
    update_wight(learnt_gate, confict_nums);
    // add complete learnt_gate into graph
    auto this_gate = graph.add_learnt_gate(m_learnt_inputs, polarity);  //!!!!!!!!

    int second_max_level_line, max_idx, second_idx;
    std::tie(second_max_level_line, max_idx, second_idx) = second_maxDecision_line(m_learnt_inputs);
    this_gate->get_pointers().first = this_gate->get_inputs()[max_idx]->num_name;
    this_gate->get_pointers().second = this_gate->get_inputs()[second_idx]->num_name;
    watching_list[1 - this_gate->get_inputs_polarity()[max_idx]][this_gate->get_inputs()[max_idx]->num_name].push_back(graph.get_gates().size() - 1);
    watching_list[1 - this_gate->get_inputs_polarity()[second_idx]][this_gate->get_inputs()[second_idx]->num_name].push_back(graph.get_gates().size() - 1);

    // backtrack
    int second_level = ls.at(second_max_level_line).level;
    //多回溯一层然后加上一层
    int decision = decision_line_name[second_level];
    int assign = ls.at(decision).assign;
    ...
    ls.at(decision).assign = assign;
    //ls.at(decision).assign = rand() % 2;
...
std::tuple<int, int, int> solver::second_maxDecision_line(std::vector<Line *> &a) 
{
  int max, second;
  ...
  return {second, max_idx, second_idx};
}
...
void solver::update_wight(const std::vector<int> &input_line, int confict_nums) {
  for (const auto &input : input_line) {
    ...
    ls.at(input).weight = temp / 2 + confict_nums / (temp * level + 1);
  }
}
