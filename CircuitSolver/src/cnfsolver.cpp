...
int solver::unit_propagate(cnf &f, std::queue<int> &bcp_que,
                           int reason_line_name)
{
  bool unit_clause_found =
      false; // whether the current iteration found a unit clause
  if (f.clauses.size() == 0)
    return 1; // formula contains no clauses, it is vacuously satisfied
  do
  {
...
int solver::apply_transform(cnf &f, std::queue<int> &bcp_que, int reason_line_name)
{
  ...
  for (int i = 0; i < f.clauses.size(); i++) // iterate over the clauses in f
  {
    ...
    for (int j = 1; j < f.clauses[i].size();
         j++) // iterate over the variables in the clause
    {
      ...
      else if (reason_line_name == abs(f.clauses[i][j])) // same polarity
      {
        f.clauses[i][0] = 1;
        flag_sat_clause++;
        if (flag_sat_clause == f.clauses.size()) // if all clauses have been removed, the formula
                              // is satisfied
        {
          return 1; // satisfied
        }
